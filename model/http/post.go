...
type Post struct {
	...
	PostImageHash 		string 					`json:"postImageHash" bson:"postImageHash" validate:"required"`
	...
	PostCategory 		string 					`json:"postCategory" bson:"postCategory" validate:"required,validPostCategory"`
	...
	PostName 			string 					`json:"postName" bson:"postName" validate:"required,min=8,max=255"`
	...
	PostDescription 	string 					`json:"postDescription" bson:"postDescription" validate:"required,min=8,max=1024"`

	// PostPrice specifies the price associated with the post.
	// The `validate:"required"` tag ensures it must be provided.
	PostPrice 			float64 				`json:"postPrice" bson:"postPrice" validate:"required"`

	// Network specifies the underlying blockchain network on which the task will be built.
	Network				string					`json:"network" bson:"network"`

	// RequiredTalent specifies the number of talents needed for the post.
	RequiredTalent 		uint64 					`json:"requiredTalent" bson:"requiredTalent"`
	...
	PostType 			post.Type 				`json:"postType" bson:"postType" validate:"required,validPostType"`
	...
	PostStatus 			post.Status 			`json:"postStatus" bson:"postStatus" validate:"required,validPostStatus"`
